...
func main() {
	...
	cfg, err := krista.LoadConfig(*cfgPath)
	if err != nil {
	...
	slog.Info("Starting krista.su...", slog.Any("config", cfg), slog.Any("version", version), slog.Any("commit", commit))

	var (
		tmplFunc krista.ExecuteTemplateFunc
	...
	s := krista.NewServer(cfg, httpClient, githubClient, md, assets, tmplFunc)
	go s.Start()
	defer s.Close()

	slog.Info("started krista.su", slog.Any("listen_addr", cfg.ListenAddr))
...
func setupLogger(cfg krista.LogConfig) {
